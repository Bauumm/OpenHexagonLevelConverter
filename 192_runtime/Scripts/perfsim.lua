...
function prefix_perfsim:update_3D()
	...
	local max_factor_3D = (
		depth_factor * ((pulse_interval - prefix_style["3D_pulse_max"]) * s_get3dSkew() * 3.6 * s_get3dSpacing() * 1.4)
	) ^ 2
	self.factor_3D = 1
		+ (depth_factor * ((pulse_interval - prefix_style_module.pulse3D) * s_get3dSkew() * 3.6 * s_get3dSpacing() * 1.4)) ^ 2
		- max_factor_3D
...
function prefix_perfsim:get_target()
	...
	local ft = (
		(0.785 * prefix_style_module.depth + 1) * (0.000461074 * prefix_perf_const + 0.000155698) * wc
		+ prefix_perf_const * (0.025 * prefix_style_module.depth + 1)
	)
